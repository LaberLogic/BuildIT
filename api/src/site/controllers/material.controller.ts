...
export const createMaterialController = async (
  req: FastifyRequest<{ Params: { siteId: string ,companyId: string}; Body: CreateMaterialDto }>,
  reply: FastifyReply,
) => {
...
export const updateMaterialController = async (
  ...
  return updateMaterialProperties(currentUser,companyId, siteId, materialId, data).match(
...
export const deleteMaterialController = async (
  req: FastifyRequest<{ Params: { siteId: string; materialId: string; companyId: string } }>,
  reply: FastifyReply,
) => {
  const { siteId, materialId,companyId } = req.params;
  const currentUser = req.user as UserObject;

  return deleteMaterialFromSite(currentUser,companyId, siteId, materialId).match(
